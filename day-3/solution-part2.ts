...
for (let line_index = 0; line_index < lines.length; line_index++) {
  ...
  // Add the numbers and symbols to the numbers and mul_symbols arrays
  numbers.push(...Array.from(line.matchAll(/\d+/g), buildPoint));
  mul_symbols.push(...Array.from(line.matchAll(/\*/g), buildPoint));
}

// Go through all the symbols to find their immediate neighbours
mul_symbols.map(symbol => {
  ...
  // Check if the symbol has more than one neighbour
  if (neighbours.length > 1) {
    // If yes, we multiply the neighbours together and add the result to the solution variable
